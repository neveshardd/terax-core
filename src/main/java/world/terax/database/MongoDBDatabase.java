...
import world.terax.database.conversor.MongoDBConversor;
...
import world.terax.reflection.acessors.MethodAccessor;
...
public class MongoDBDatabase extends Database {
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      MongoCollection<Document> collection = this.database.getCollection("CoreNetworkBooster");
      for (String mg : Core.minigames) {
        if (collection.find(new BasicDBObject("_id", mg)).first() == null) {
          this.executor.execute(() -> collection.insertOne(new Document("_id", mg).append("booster", "neveshardd").append("multiplier", 1.0).append("expires", 0L)));
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    this.executor.execute(() -> this.database.getCollection("CoreNetworkBooster")
  ...
  public NetworkBooster getBooster(String minigame) {
    try {
      Document document = this.executor.submit(() -> this.database.getCollection("CoreNetworkBooster").find(new BasicDBObject("_id", minigame)).first()).get();
      if (document != null) {
