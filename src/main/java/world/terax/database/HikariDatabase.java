package world.terax.database;

import com.zaxxer.hikari.HikariConfig;
import com.zaxxer.hikari.HikariDataSource;
import world.terax.Core;
import world.terax.Manager;
import world.terax.booster.NetworkBooster;
...
import world.terax.database.exception.ProfileLoadException;
...
public class HikariDatabase extends Database {
  ...
  public HikariDatabase(String host, String port, String dbname, String username, String password, boolean mariadb) {
    ...
    this.update(
        "CREATE TABLE IF NOT EXISTS `CoreNetworkBooster` (`id` VARCHAR(32), `booster` TEXT, `multiplier` DOUBLE, `expires` LONG, PRIMARY KEY(`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE utf8_bin;");
  ...
  public void setupBoosters() {
    if (!Manager.BUNGEE) {
      for (String mg : Core.minigames) {
        if (query("SELECT * FROM `CoreNetworkBooster` WHERE `id` = ?", mg) == null) {
          execute("INSERT INTO `CoreNetworkBooster` VALUES (?, ?, ?, ?)", mg, "neveshardd", 1.0, 0L);
        }
      }
    }
  }
  ...
  public void setBooster(String minigame, String booster, double multiplier, long expires) {
    execute("UPDATE `CoreNetworkBooster` SET `booster` = ?, `multiplier` = ?, `expires` = ? WHERE `id` = ?", booster, multiplier, expires, minigame);
  }
  
  @Override
  public NetworkBooster getBooster(String minigame) {
    try (CachedRowSet rs = query("SELECT * FROM `CoreNetworkBooster` WHERE `id` = ?", minigame)) {
      if (rs != null) {
  ...
  public String exists(String name) {
    ...
    public void openConnection() {
        ...
        // Detecta se estamos no BungeeCord
        boolean isBungee = false;
        try {
            Class.forName("net.md_5.bungee.api.plugin.Plugin"); // tenta achar classe do Bungee
            isBungee = true;
        } catch (ClassNotFoundException ignored) {}

        // Define driver dependendo do ambiente
        if (this.mariadb) {
            config.setDriverClassName("org.mariadb.jdbc.Driver");
        } else if (isBungee) {
            config.setDriverClassName("com.mysql.cj.jdbc.Driver"); // driver novo para Bungee
        } else {
            config.setDriverClassName("com.mysql.jdbc.Driver"); // driver antigo para Spigot/normal
        }

        // Monta URL
        String jdbcPrefix = this.mariadb ? "jdbc:mariadb://" : "jdbc:mysql://";
        config.setJdbcUrl(jdbcPrefix + this.host + ":" + this.port + "/" + this.dbname);
        ...
        LOGGER.info("Conectado ao MySQL/MariaDB! Ambiente Bungee: " + isBungee);
    }
