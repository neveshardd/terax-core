...
import static world.terax.party.PartyRole.LEADER;
  ...
  public void perform(CommandSender sender, String[] args) {
    ...
    if (action.equalsIgnoreCase("abrir")) {
      ...
      if (!party.isLeader(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não é o Líder da Party."));
        return;
      }
      
      if (party.isOpen()) {
        player.sendMessage(new TextComponent("§cSua party já é pública."));
        return;
      }
      
      party.setIsOpen(true);
      player.sendMessage(new TextComponent("§aVocê abriu a party para qualquer jogador."));
      ...
      if (!party.isOpen()) {
        player.sendMessage(new TextComponent("§cSua party já é privada."));
        return;
      }
      
      party.setIsOpen(false);
      player.sendMessage(new TextComponent("§cVocê fechou a party para apenas convidados."));
    } else if (action.equalsIgnoreCase("entrar")) {
      if (args.length == 1) {
        player.sendMessage(new TextComponent("§cUtilize /party entrar [jogador]"));
        return;
      }
      
      String target = args[1];
      if (target.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode entrar na party de você mesmo."));
        return;
      }
      
      BungeeParty party = BungeePartyManager.getMemberParty(player.getName());
      if (party != null) {
        player.sendMessage(new TextComponent("§cVocê já pertence a uma Party."));
        return;
      }
      
      party = BungeePartyManager.getLeaderParty(target);
      if (party == null) {
        player.sendMessage(new TextComponent("§c" + Manager.getCurrent(target) + " não é um Líder de Party."));
        return;
      }
      
      target = party.getName(target);
      if (!party.isOpen()) {
        player.sendMessage(new TextComponent("§cA Party de " + Manager.getCurrent(target) + " está fechada apenas para convidados."));
        return;
      }
      
      if (!party.canJoin()) {
        player.sendMessage(new TextComponent("§cA Party de " + Manager.getCurrent(target) + " está lotada."));
        return;
      }
      ...
      if (args.length == 1) {
        player.sendMessage(new TextComponent("§cUtilize /party aceitar [jogador]"));
        return;
      }
      
      String target = args[1];
      if (target.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode aceitar convites de você mesmo."));
        return;
      }
      ...
      if (!party.isInvited(player.getName())) {
        player.sendMessage(new TextComponent("§c" + Manager.getCurrent(target) + " não convidou você para Party."));
        return;
      }
      ...
      player.sendMessage(new TextComponent("§aVocê puxou todos os jogadores para o seu servidor."));
      ...
      party.delete();
      player.sendMessage(new TextComponent("§aVocê deletou a Party."));
    } else if (action.equalsIgnoreCase("expulsar")) {
      if (args.length == 1) {
        player.sendMessage(new TextComponent("§cUtilize /party expulsar [jogador]"));
        return;
      }
      
      BungeeParty party = BungeePartyManager.getLeaderParty(player.getName());
      if (party == null) {
        player.sendMessage(new TextComponent("§cVocê não é um Líder de Party."));
        return;
      }
      
      String target = args[1];
      if (target.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode se expulsar."));
        return;
      }
      
      if (!party.isMember(target)) {
        player.sendMessage(new TextComponent("§cEsse jogador não pertence a sua Party."));
        return;
      }
      ...
      if (args.length == 1) {
        player.sendMessage(new TextComponent("§cUtilize /party negar [jogador]"));
        return;
      }
      
      String target = args[1];
      if (target.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode negar convites de você mesmo."));
        return;
      }
      ...
      player.sendMessage(new TextComponent("§aVocê saiu da Party!"));
    } else if (action.equalsIgnoreCase("transferir")) {
      if (args.length == 1) {
        player.sendMessage(new TextComponent("§cUtilize /party transferir [jogador]"));
        return;
      }
      ...
      String target = args[1];
      if (target.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode transferir a Party para você mesmo."));
        return;
      }
      ...
      if (action.equalsIgnoreCase("convidar")) {
        if (args.length == 1) {
          player.sendMessage(new TextComponent("§cUtilize /party convidar [jogador]"));
          return;
        }
      ...
      if (action.equalsIgnoreCase(player.getName())) {
        player.sendMessage(new TextComponent("§cVocê não pode enviar convites para você mesmo."));
        return;
      }
      ...
      if (!party.isLeader(player.getName())) {
        player.sendMessage(new TextComponent("§cApenas o Líder da Party pode enviar convites!"));
        return;
      }
      
      if (!party.canJoin()) {
        player.sendMessage(new TextComponent("§cA sua Party está lotada."));
        return;
      }
      
      if (party.isInvited(action)) {
        player.sendMessage(new TextComponent("§cVocê já enviou um convite para " + Manager.getCurrent(action) + "."));
        return;
      }
      
      if (BungeePartyManager.getMemberParty(action) != null) {
        player.sendMessage(new TextComponent("§c" + Manager.getCurrent(action) + " já pertence a uma Party."));
        return;
      }
