package world.terax.bungee;

import com.google.common.io.ByteArrayDataOutput;
import com.google.common.io.ByteStreams;
import world.terax.bungee.cmd.Commands;
import world.terax.bungee.listener.Listeners;
...
public class Bungee extends Plugin {
  ...
  public static void applyFake(ProxiedPlayer player, String fakeName, String role, String skin) {
    player.disconnect(new TextComponent(StringUtils.formatColors(getInstance().getConfig().getString("fake.kick-apply")).replace("\\n", "\n")));
  ...
  public static void removeFake(ProxiedPlayer player) {
    player.disconnect(new TextComponent(StringUtils.formatColors(getInstance().getConfig().getString("fake.kick-remove")).replace("\\n", "\n")));
  ...
  public void saveDefaultConfig() {
    for (String fileName : new String[]{"config", "roles", "utils"}) {
      File file = new File("plugins/Core/" + fileName + ".yml");
      if (!file.exists()) {
  ...
  private void setupRoles() {
    try {
      if (utils.get("fake.role") instanceof String) {
        ...
        YamlConfiguration.getProvider(YamlConfiguration.class).save(utils, new File("plugins/Core/utils.yml"));
      }
    } catch (IOException ex) {
      ex.printStackTrace();
    }
