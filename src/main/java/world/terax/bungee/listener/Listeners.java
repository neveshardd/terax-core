package world.terax.bungee.listener;

import com.google.common.io.ByteArrayDataInput;
import com.google.common.io.ByteArrayDataOutput;
import com.google.common.io.ByteStreams;
import world.terax.bungee.Bungee;
import world.terax.bungee.cmd.StaffChatCommand;
    ...
    if (evt.getSender() instanceof ProxiedPlayer) {
        ...
        if (evt.isCommand()) {
            ...
            if (COMMAND_MAP.get(ProxyServer.getInstance().getPluginManager()).containsKey("lobby") && command.equals("lobby") && this
                ...
                player.sendMessage(new TextComponent("§aVocê tem certeza? Utilize /lobby novamente para voltar ao lobby."));
                ...
                if (!this.canReceiveTell(args[1].toLowerCase())) {
                    evt.setCancelled(true);
                    player.sendMessage(new TextComponent("§cEste usuário desativou as mensagens privadas."));
                }
            }
            ...
            if (StaffChatCommand.STAFFS.contains(player)) {
                ...
                BungeeCord.getInstance().getPlayers().stream().filter(playerx -> playerx.hasPermission("command.staffchat")).forEach(playerx -> {
                    playerx.sendMessage(new TextComponent("§6[SC] " +
